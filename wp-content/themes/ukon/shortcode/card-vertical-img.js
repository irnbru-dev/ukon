(function() {
    tinymce.PluginManager.add('card_vertical_img_button', function( editor, url ) {
        editor.addButton('card_vertical_img_button', {
            text: 'Карточка вертикальня',
                    ...
                    onsubmit: (e) => {
                        ...
                        for (let el = 1; el <= cardNum; el++) {
                            body.push(
                                ...
                                {
                                    type: 'button',
                                    name: 'bg_upload_button' + el,
                                    label: '',
                                    text: 'Загрузить фон',
                                    classes: 'bg_upload_button',
                                },
                                {
                                    type: 'textbox',
                                    name: 'title' + el,
                        ...
                        editor.windowManager.open({
                            ...
                            onsubmit: (e) => {
                                ...
                                for (let el = 1; el <= cardNum; el++) {
                                    editor.insertContent('[card-vertical-img link="' + e.data['link' + el] + '" icon="' + e.data['icon' + el] + '" background="' + e.data['background' + el] + '" title="' + e.data['title' + el] + '"]<p>' + e.data['content' + el] + '</p>[/card-vertical-img]');
                                }
    ...
    jQuery(document).ready(function ($) {
        ...
        $(document).on('click', '.mce-bg_upload_button', upload_image_tinymce);
